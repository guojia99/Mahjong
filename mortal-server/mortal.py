...
def main():
    ...
    state = load_checkpoint(config['control']['state_file'], map_location=torch.device('cpu'))
    cfg = state['config']
    ...
    if review_mode:
        ...
        grp_state = load_checkpoint(config['grp']['state_file'], map_location=torch.device('cpu'))
