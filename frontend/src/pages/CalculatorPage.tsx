...
import { Pai, Block, BlockType, PositionType, TSUMO, RON, RIICHI, DOUBLE_RIICHI, IPPATSU, HAITEI_RAOYUE, HOUTEI_RAOYUI, RINNSHANN_KAIHOU, CHANKAN, TENHOU, CHIIHOU, State } from '@/mahjong-calc/types';
...
const WIND_OPTIONS = [{ value: 'east', label: '东' }, { value: 'south', label: '南' }, { value: 'west', label: '西' }, { value: 'north', label: '北' }];
...
function TileImg({ name, onClick, disabled, highlight, small }: { name: string; onClick?: () => void; disabled?: boolean; highlight?: boolean; small?: boolean }) {
  const src = `/marjongs/${name}.webp`;
  const isH = name.startsWith('H');
  return (
    <button type="button" onClick={disabled ? undefined : onClick} style={{
      border: 'none', background: 'transparent', padding: 0, cursor: disabled ? 'not-allowed' : onClick ? 'pointer' : 'default',
      opacity: disabled ? 0.3 : 1, filter: highlight ? 'drop-shadow(0 0 4px #e65100)' : 'none',
      flexShrink: 0, lineHeight: 0,
    }}>
      <img src={src} alt={name} draggable={false}
        style={{ height: isH ? (small ? '2rem' : '2.5rem') : (small ? '2.5rem' : '3.5rem'), width: isH ? 'auto' : undefined, borderRadius: '0.25rem' }} />
    </button>
  );
}
...
function FuruBlock({ item, onRemove }: { item: { type: string; name: string; red?: boolean }; onRemove: () => void }) {
  const tiles = useMemo(() => {
    const rt: string[] = [];
    if (item.type === 'pon') { rt.push('H' + item.name); rt.push(item.name); rt.push(item.name); }
    else if (item.type === 'kan') { rt.push('H' + item.name); rt.push(item.name); rt.push(item.name); rt.push(item.name); }
    else if (item.type === 'ankan') { rt.push('B'); rt.push(item.name); rt.push(item.name); rt.push('B'); }
      ...
      if (item.red) {
        ...
        tiles[redIdx] = '0' + tp;
        if (redIdx === 0) { [tiles[0], tiles[1]] = [tiles[1], tiles[0]]; }
      }
      rt.push('H' + tiles[0]);
      rt.push(tiles[1]);
      rt.push(tiles[2]);
  ...
  return (
    <div onClick={onRemove} style={{ display: 'inline-flex', alignItems: 'flex-end', gap: '1px', background: 'rgba(255,243,224,0.3)', padding: '2px 4px', borderRadius: '0.5rem', cursor: 'pointer', transition: 'opacity 0.15s' }}
...
const selectStyle: React.CSSProperties = { padding: '0.375rem 0.75rem', fontSize: '0.8rem', borderRadius: '0.5rem', border: '2px solid var(--color-border)', background: 'white', color: 'var(--color-text)', outline: 'none', cursor: 'pointer' };
const inputStyle: React.CSSProperties = { width: '4rem', padding: '0.375rem 0.5rem', borderRadius: '0.5rem', border: '2px solid var(--color-border)', fontSize: '0.8rem', outline: 'none' };
...
export default function CalculatorPage() {
  ...
  const paiLeft = useMemo(() => {
    ...
    for (const p of hand) { const k = normalKey(p); pl[k] = (pl[k] || 0) - 1; }
  ...
  const hasRed = useMemo(() => {
    ...
    for (const p of hand) { if (p.startsWith('0')) { cnt[p[1]] = (cnt[p[1]] || 0) + 1; } }
    for (const f of furo) { if (f.red) cnt[f.name[1]] = (cnt[f.name[1]] || 0) + 1; }
    return cnt;
  }, [hand, furo]);
  ...
  const tileDisable = useMemo(() => {
    ...
    if (popup === 'hand') {
      ...
      for (const p of RED_TILES) { const k = '5' + p[1]; if (getAvail(k) > 0 && (hasRed[p[1]] || 0) < 1) rt[p] = false; }
    } else if (popup === 'dora') {
      ...
      if (furuMode === 'chi') {
        ...
        for (const p of RED_TILES) { const tp = p[1]; if (getAvail('3' + tp) > 0 && getAvail('4' + tp) > 0 && (hasRed[tp] || 0) < 1) rt[p] = false; }
  ...
  const yakuDisable = useMemo(() => {
    ...
    for (const b of furo) { if (b.type !== 'ankan') isMenzen = false; if (b.type === 'kan' || b.type === 'ankan') haveKan = true; }
    if (agariWay === 'tsumo') { s.chankan = true; s.houte = true; }
    if (agariWay === 'ron') { s.rinnshann = true; s.haite = true; s.tenhou = true; s.chiihou = true; }
    if (!isMenzen) { s.riichi = true; s['double-riichi'] = true; s.ippatsu = true; }
  ...
  useEffect(() => {
    if (canCalc) {
      ...
      const tAgariPai = tPai[tPai.length - 1]; tPai.pop();
      ...
      if (agariWay === 'tsumo') { r.point1 += 100 * ponba; r.point2 += 100 * ponba; }
  ...
  const removeTile = useCallback((arr: string[], idx: number, setter: (v: string[]) => void) => { setter(arr.filter((_, i) => i !== idx)); }, []);
  const removeFuru = useCallback((idx: number) => { setFuro(p => p.filter((_, i) => i !== idx)); }, []);
  const clearAll = () => { setHand([]); setFuro([]); setDora([]); setUra([]); setYakus([]); setAgariWay('tsumo'); setField('east'); setSeat('east'); setPonba(0); setPopup(null); setChiRedPick(null); };

  const toggleYaku = (v: string) => { if (yakuDisable[v]) return; setYakus(p => p.includes(v) ? p.filter(x => x !== v) : [...p, v]); };
  ...
  const ZoneBox = ({ label, target, items, isFuru, hint }: { label: string; target: PopupTarget; items: string[]; isFuru?: boolean; hint?: string }) => {
    const isEmpty = isFuru ? furo.length === 0 : items.length === 0;
    return (
      <div>
        <div style={{ fontSize: '0.75rem', color: 'var(--color-text-light)', marginBottom: '0.25rem' }}>{label}</div>
        <div
        ...
        >
          ...
          ) : isFuru ? furo.map((f, i) => <FuruBlock key={i} item={f} onRemove={(e) => { e.stopPropagation(); removeFuru(i); }} />) :
            items.map((p, i) => <TileImg key={i} name={p} onClick={(e) => { e.stopPropagation(); removeTile(items, i, target === 'dora' ? setDora : target === 'ura' ? setUra : setHand); }} highlight={i === items.length - 1 && target === 'hand'} />)}
        </div>
      </div>
    );
  };
  ...
  const TilePopup = () => {
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(0,0,0,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
      onClick={() => { setPopup(null); setChiRedPick(null); }}>
      <div style={{ background: 'white', borderRadius: '1.25rem', padding: '1rem 1.25rem', boxShadow: '0 8px 30px rgba(0,0,0,0.15)', maxWidth: '95vw', minWidth: '20rem' }}
        onClick={e => e.stopPropagation()}>

        {_chiRedPick && (
          <div style={{ marginBottom: '0.75rem' }}>
            <div style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--color-text)', marginBottom: '0.5rem' }}>选择吃的顺子：</div>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              {[3, 4, 5].map(n => {
                ...
                if (n === 5) { [displayTiles[0], displayTiles[1]] = [displayTiles[1], displayTiles[0]]; }
                return (
                  <button key={n} disabled={!avail} onClick={() => addChiRed(_chiRedPick!, n)}
                    style={{
                      padding: '0.3rem 0.5rem', borderRadius: '0.5rem', border: '1px solid var(--color-border)',
                      background: avail ? 'var(--color-primary-light)' : '#f5f5f5',
                      cursor: avail ? 'pointer' : 'not-allowed', opacity: avail ? 1 : 0.4,
                      display: 'flex', gap: '1px', alignItems: 'center', transition: 'all 0.15s',
                    }}>
                    {displayTiles.map(t => (
        ...
        {_popup === 'furu' && !_chiRedPick && (
          <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.75rem' }}>
            {FURU_TYPES.map(o => (
              <button key={o.value} onClick={e => { e.stopPropagation(); setFuroMode(o.value); }}
                style={{
                  padding: '0.3rem 0.75rem', borderRadius: '0.5rem', border: _furoMode === o.value ? '2px solid var(--color-primary)' : '1px solid var(--color-border)',
        ...
        {_previewTiles.length > 0 && !_chiRedPick && (
          <div style={{ marginBottom: '0.5rem', paddingBottom: '0.5rem', borderBottom: '1px dashed var(--color-border)' }}>
            <div style={{ fontSize: '0.7rem', color: 'var(--color-text-light)', marginBottom: '0.25rem' }}>当前选择 ({_previewTiles.length})</div>
        ...
        {!_chiRedPick && (
          <div>
            {TILE_ROWS.map((row, ri) => (
              <div key={ri} style={{ display: 'flex', gap: '2px', marginBottom: '2px', justifyContent: 'center' }}>
                {row.map(p => <TileImg key={p} name={p} onClick={() => addTile(_popup!, p)} disabled={_tileDisable[p]} />)}
              </div>
            ))}
            {_popup === 'hand' && (
              <div style={{ display: 'flex', gap: '2px', marginBottom: '2px', justifyContent: 'center' }}>
                {RED_TILES.map(p => <TileImg key={p} name={p} onClick={() => addTile(_popup!, p)} disabled={_tileDisable[p]} />)}
              </div>
            )}
            {_popup === 'furu' && _furoMode === 'chi' && (
              <div style={{ display: 'flex', gap: '2px', marginBottom: '2px', justifyContent: 'center' }}>
                {RED_TILES.map(p => <TileImg key={p} name={p} onClick={() => setChiRedPick(p)} disabled={_tileDisable[p]} />)}
              </div>
            )}
          </div>
        )}
        ...
        <div style={{ textAlign: 'center', marginTop: '0.5rem' }}>
          <button onClick={() => { setPopup(null); setChiRedPick(null); }}
            style={{ padding: '0.25rem 1rem', borderRadius: '0.5rem', border: '1px solid var(--color-border)', background: 'transparent', fontSize: '0.75rem', cursor: 'pointer', color: 'var(--color-text-light)' }}>
            {_chiRedPick ? '返回' : '关闭'}
          </button>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col items-center gap-5">
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', justifyContent: 'center', alignItems: 'flex-start', width: '100%', maxWidth: '48rem' }}>
        <div style={{ display: 'flex', gap: '1rem', width: '100%' }}>
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '0.5rem', background: 'white', borderRadius: '1rem', padding: '0.875rem 1rem', border: '1px solid var(--color-border)' }}>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              {[
                { label: '场风', value: field, onChange: setField },
                { label: '自风', value: seat, onChange: setSeat },
              ].map(({ label, value, onChange }) => (
                <div key={label} style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.8rem' }}>
                  <span style={{ color: 'var(--color-text-light)', whiteSpace: 'nowrap' }}>{label}</span>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.8rem' }}>
              ...
              <input type="number" value={ponba} onChange={e => setPonba(Math.max(0, parseInt(e.target.value) || 0))} min={0} style={inputStyle} />
            </div>
          </div>
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '0.375rem', background: 'white', borderRadius: '1rem', padding: '0.875rem 1rem', border: '1px solid var(--color-border)' }}>
            {YAKU_OPTIONS.map(o => (
              <label key={o.value} style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', fontSize: '0.8rem', cursor: yakuDisable[o.value] ? 'not-allowed' : 'pointer', opacity: yakuDisable[o.value] ? 0.4 : 1, userSelect: 'none' }}>
                <input type="checkbox" checked={yakus.includes(o.value)} disabled={yakuDisable[o.value]} onChange={() => toggleYaku(o.value)} />
                {o.label}
              </label>
            ))}
          </div>
        ...
        {result && (
          <div style={{ background: 'white', border: '2px solid var(--color-border)', borderRadius: '1rem', padding: '1rem 1.25rem', minWidth: '13rem' }}>
            {isTrueAgari ? (
              <>
                <div style={{ fontSize: '2.5rem', fontWeight: 800, color: 'var(--color-primary-dark)', lineHeight: 1.1 }}>
                  {result.pointType === 2 ? `${result.point1} / ${result.point2}` : result.pointType === 0 ? `${result.point1} ALL` : result.point1}
                </div>
                <div style={{ fontSize: '1rem', color: 'var(--color-text)', marginTop: '0.25rem', fontWeight: 600 }}>
                  {result.isYakuman ? `${result.han}倍役满` : `${result.han}翻${needFu ? ` ${result.fu}符` : ''} ${manName}`}
                </div>
                <ul style={{ margin: '0.5rem 0 0', paddingLeft: '1.25rem', fontSize: '0.8rem', color: 'var(--color-text)' }}>
                ...
                {needFu && result.fuMessages.length > 0 && (
                  <div style={{ borderTop: '1px dashed var(--color-border)', paddingTop: '0.5rem', marginTop: '0.5rem' }}>
                    <div style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--color-text-light)', marginBottom: '0.25rem' }}>符计算过程</div>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.375rem' }}>
                      {result.fuMessages.map((m, i) => (
                        <span key={i} style={{ background: '#f0f7ff', color: '#1565c0', padding: '0.2rem 0.5rem', borderRadius: '0.5rem', fontSize: '0.75rem', fontWeight: 500 }}>{m}</span>
                      ))}
                    </div>
                  </div>
                )}
      ...
      <div style={{ width: '100%', maxWidth: '40rem', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
        <ZoneBox label={`手牌 (${hand.length}/${14 - furo.length * 3})`} target="hand" items={hand} hint="手牌" />
