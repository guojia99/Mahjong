...
export class MenzenTsumo implements Yaku {
  test(h: HandSet, r: Rule) { return test(h.flag, (1 << 17) | (1 << 19)) ? 1 : 0; }
  getName() { return '门前清自摸和'; }
}
export class TanYao implements Yaku {
  test(h: HandSet, r: Rule) {
    if (!r.shiDuan && !test(h.flag, 1 << 19)) return 0;
...
export class YakuhaiHako implements Yaku {
  test(h: HandSet, r: Rule) { for (const b of h.blocks) if (b.pType === 'z' && b.num === 5) return 1; return 0; }
  getName() { return '役牌 - 白'; }
}
export class YakuhaiHatsu implements Yaku {
  test(h: HandSet, r: Rule) { for (const b of h.blocks) if (b.pType === 'z' && b.num === 6) return 1; return 0; }
  getName() { return '役牌 - 发'; }
}
export class YakuhaiCyuu implements Yaku {
  test(h: HandSet, r: Rule) { for (const b of h.blocks) if (b.pType === 'z' && b.num === 7) return 1; return 0; }
  getName() { return '役牌 - 中'; }
}
...
export class AkaDora implements Yaku {
  test(h: HandSet, r: Rule) { return h.redCnt; }
  getName() { return '赤宝牌'; }
}
export class Toitoi implements Yaku {
  test(h: HandSet, r: Rule) { for (const b of h.blocks) if (b.bType === BlockType.SEQ) return 0; return 2; }
  getName() { return '对对和'; }
}
export class Sanankou implements Yaku {
  test(h: HandSet, r: Rule) { let c = 0; for (const b of h.blocks) if (b.bType !== BlockType.SEQ && !b.isOpen) c++; return c === 3 ? 2 : 0; }
  getName() { return '三暗刻'; }
}
...
export class Sankantsu implements Yaku {
  test(h: HandSet, r: Rule) { let c = 0; for (const b of h.blocks) if (b.bType === BlockType.QUAD) c++; return c === 3 ? 2 : 0; }
  getName() { return '三杠子'; }
}
...
export class Daisangen implements Yaku {
  test(h: HandSet, r: Rule) { let c = 0; for (const b of h.blocks) if (b.pType === 'z' && 5 <= b.num && b.num <= 7) c++; return c === 3 ? 1 : 0; }
  getName() { return '大三元'; }
}
...
export class SuuankouTanki implements Yaku {
  test(h: HandSet, r: Rule) {
    if (!test(h.flag, 1 << 19)) return 0;
    let c = 0; for (const b of h.blocks) if (b.bType !== BlockType.SEQ && !b.isOpen) c++;
    return (c === 4 && h.agariPai.equalTo(h.pair.getPai()[0])) ? (r.duoBeiYiMan ? 2 : 1) : 0;
  }
...
export class Daisuushi implements Yaku {
  test(h: HandSet, r: Rule) {
    let c = 0; for (const b of h.blocks) if (b.pType === 'z' && 1 <= b.num && b.num <= 4) c++;
    return c === 4 ? (r.duoBeiYiMan ? 2 : 1) : 0;
  }
  getName() { return '大四喜'; }
}
...
export class ChuurenPoutou implements Yaku {
  test(h: HandSet, r: Rule) {
    if (!test(h.flag, 1 << 19)) return 0;
    if (!new Chiniisou().test(h, r)) return 0;
...
export class JunseiChuurenPoutou implements Yaku {
  test(h: HandSet, r: Rule) {
    ...
    let mulNum = -1;
    for (let i = 1; i <= 9; i++) { if (cnt[i] < need[i] || cnt[i] > need[i] + 1) return 0; if (cnt[i] === need[i] + 1) mulNum = i; }
    return mulNum === h.agariPai.num ? (r.duoBeiYiMan ? 2 : 1) : 0;
  }
  getName() { return '纯正九莲宝灯'; }
}
export class Suukantsu implements Yaku {
  test(h: HandSet, r: Rule) { for (const b of h.blocks) if (b.bType !== BlockType.QUAD) return 0; return 1; }
  getName() { return '四杠子'; }
}
export class Tenhou implements Yaku {
  test(h: HandSet, r: Rule) { return (!test(h.flag, 1 << 19) || !test(h.flag, 1 << 10)) ? 0 : 1; }
  getName() { return '天和'; }
}
export class Chiihou implements Yaku {
  test(h: HandSet, r: Rule) { return (!test(h.flag, 1 << 19) || !test(h.flag, 1 << 11)) ? 0 : 1; }
  getName() { return '地和'; }
}
