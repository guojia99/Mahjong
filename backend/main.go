...
【方式 A】账号密码（推荐，已适配国服 WebGL 网页版）
  ...
  若报 code=151 且 version_str 为空：多为 WebGL 版本不匹配，可设
    MAJSOUL_WEBGL_RESOURCE=0.16.238  MAJSOUL_WEBGL_PACKAGE=4.0.44
...
func run(cmd *cobra.Command, args []string) error {
	...
	api.Use(middleware.QueryCache())
	{
		...
		leagues := api.Group("/leagues")
		{
			...
			leagues.GET("/seasons/:pk/stats/", handlers.LeagueSeasonStats)
			...
			leagues.GET("/stages/:pk/stats/", handlers.LeagueStageStats)
