...
func PlayerAvatar(c *gin.Context) {
	pk := c.Param("pk")
	var player models.Player
	if err := config.DB.Select("id, avatar").Where("id = ?", pk).First(&player).Error; err != nil {
		respondError(c, http.StatusNotFound, "Not found")
		return
	}
	respondOK(c, gin.H{"avatar": player.Avatar})
}
...
func PlayerCreate(c *gin.Context) {
	...
	if req.ExtraInfo != nil {
		var err error
		extraInfo, err = models.NewJSONField(req.ExtraInfo)
		if err != nil {
			respondError(c, http.StatusBadRequest, "Invalid extra_info")
			return
		}
	}
