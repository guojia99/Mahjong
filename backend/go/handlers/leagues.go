...
func leagueStringListToJSONField(list []string) models.JSONField {
	...
	return models.JSONField{"items": items}
}
...
func leagueJSONFieldToStringList(jf models.JSONField) []string {
	...
	if items, ok := jf["items"].([]interface{}); ok {
...
func LeagueSeriesUpdate(c *gin.Context) {
	pk := c.Param("pk")
	var s models.LeagueSeries
	if err := config.DB.Where("id = ?", pk).First(&s); err != nil {
		respondError(c, http.StatusNotFound, "Not found")
		return
	}
...
func LeagueSeasonCreate(c *gin.Context) {
	...
	seriesPK := c.Param("pk")
	var series models.LeagueSeries
	if err := config.DB.Where("id = ?", seriesPK).First(&series); err != nil {
...
func LeagueSeasonUpdate(c *gin.Context) {
	pk := c.Param("pk")
	var season models.LeagueSeason
	if err := config.DB.Where("id = ?", pk).First(&season); err != nil {
		respondError(c, http.StatusNotFound, "Not found")
		return
	}
	var req map[string]interface{}
	...
	config.DB.Preload("Series").Preload("Stages").First(&season, "id = ?", pk)
	respondOK(c, serializeLeagueSeasonDetailFull(&season))
}
...
func LeagueSeasonStart(c *gin.Context) {
	pk := c.Param("pk")
	var season models.LeagueSeason
	if err := config.DB.Preload("SeasonPlayers").Where("id = ?", pk).First(&season); err != nil {
		respondError(c, http.StatusNotFound, "Not found")
		return
	}
	if season.Status != "registration" {
...
func LeagueSeriesUploadLogo(c *gin.Context) {
	respondError(c, http.StatusNotImplemented, "Logo upload not implemented")
}
...
func LeagueUploadMarkdownImage(c *gin.Context) {
	respondError(c, http.StatusNotImplemented, "Markdown image upload not implemented")
}
...
func LeagueApplyStagePromotion(c *gin.Context) {
	...
	winnersPromote := 4
	if v, ok := rules["winners_promote"]; ok {
		if n, ok := v.(float64); ok {
			winnersPromote = int(n)
		}
	}
	losersEliminate := 4
	if v, ok := rules["losers_eliminate"]; ok {
		if n, ok := v.(float64); ok {
