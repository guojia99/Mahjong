...
func serializeGameList(game *models.Game) gin.H {
	...
	if game.PaipuData != nil && len(game.PaipuData) > 0 {
		hasPaipuData = true
		if actions, ok := game.PaipuData["actions"]; ok {
			if arr, ok := actions.([]interface{}); ok && len(arr) > 0 {
				paipuHasActions = true
			}
		}
		if !paipuHasActions {
			if nested, ok := game.PaipuData["majsoul_record_detail"]; ok {
...
func OnlineGameParse(c *gin.Context) {
	respondError(c, http.StatusNotImplemented, "Online game parsing requires Python majsoul service")
}
...
func OnlineGameImport(c *gin.Context) {
	respondError(c, http.StatusNotImplemented, "Online game import requires Python majsoul service")
}

func OnlineGameRetry(c *gin.Context) {
	respondError(c, http.StatusNotImplemented, "Online game retry requires Python majsoul service")
}
...
func serializeHandRecordList(hr *models.HandRecord) gin.H {
	...
	return gin.H{
		"id":            hr.ID,
		"player":        getPlayerBrief(hr.Player),
		"record_type":   hr.RecordType,
		"yakuman_names": hr.YakumanNames,
...
func HandRecordCreate(c *gin.Context) {
	...
	if len(req.YakumanNames) == 0 || req.YakumanNames == nil {
...
func StartingHands(c *gin.Context) {
	respondOK(c, gin.H{
		"count": 0, "page": 1, "page_size": 20, "results": []interface{}{},
	})
}
